...
class PECBNet_3D(nn.Module):
    """Photon enrichment with Temporal Coupled Propagation Network. Temporal sparsity (TS)
    ...
    def __init__(self,
        ...
        self.conv_last = nn.Conv2d(64, self.upscale_time, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        for i in range(t - 1, -1, -1):
            ...
            out_b.insert(0, feat_prop)

        # forward branch
        out_f = x.new_zeros(b, t * self.upscale_time, chn, h, w)
        ...
        for i in range(0, t):
            ...
            feat_prop = torch.cat([x_i, out_b[i], feat_prop], dim=1)
            ...
            for c in range(self.upscale_time):
                out_f[:,i * self.upscale_time + c,:,:,:] = out[:,c,:,:] + base
